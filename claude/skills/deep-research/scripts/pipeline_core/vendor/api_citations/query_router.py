...
# Type definitions
# local vendor patch: keyless-only chains — 'gemini_grounded' removed
# (web/industry coverage moves to the pipeline-level web-scout worker) and
# 'openalex' added (upstream defined the client but never routed to it).
QueryType = Literal['academic', 'industry', 'mixed']
APIName = Literal['crossref', 'semantic_scholar', 'openalex']
...
class QueryRouter:
    ...
    def get_api_chain(self, query_type: QueryType) -> List[APIName]:
        ...
        API Priority Chains (local keyless patch):
        - industry: Semantic Scholar → OpenAlex → Crossref
        - academic: Crossref → OpenAlex → Semantic Scholar
        - mixed: Semantic Scholar → Crossref → OpenAlex
        """
        # local vendor patch: keyless chains. gemini_grounded dropped
        # (pipeline-level web-scout worker covers web/industry sources);
        # openalex inserted — upstream never emitted it from any chain.
        if query_type == 'industry':
            return ['semantic_scholar', 'openalex', 'crossref']

        elif query_type == 'academic':
            # Prioritize academic sources for scholarly queries
            return ['crossref', 'openalex', 'semantic_scholar']

        else:  # mixed
            # Balanced approach for mixed queries
            return ['semantic_scholar', 'crossref', 'openalex']
