...
# Find staged .md files in the memory vault.
#
# This is the THIRD time this gate has silently died. First LAB-215 (2026-07-13): the
# legacy TOOL_INPUT env was always empty. Then the 2026-09-10 de-monorepo: the path below
# was "${CLAUDE_PROJECT_DIR:-.}/submodules/memory", and the vault stopped being a submodule
# of anything — so the -d test missed under EVERY project root and the hook exited 0 on
# every commit, validating nothing.
#
# The failure mode is the same each time and is what makes it recur: a gate whose skip path
# and success path are both "exit 0, print nothing". If you change this line, prove it by
# staging a frontmatter-less .md in the vault and watching the commit get BLOCKED — never by
# observing that the hook ran without error.
